...
from siteplan_qualitycontrol.utils import get_zoning_url_attribute, modify_zoning_url,get_and_modify_county_coordinates_url
...
def get_zone_info_city(
    ...
    city_url_attribute = get_zoning_url_attribute(city)
    zone_url = city_url_attribute["zoning_url"]
    loc = (x_min, x_max, y_min, y_max)
    zone_url = modify_zoning_url(zone_url, loc)
    zone_attribute = city_url_attribute["zoning_attribute"]
    ...
    if len(results) == 0:
        ...
        zone = results[0]['attributes'][zone_attribute]
...
def get_zone_info_county(
    ...
    # in maricopa county, there is different link to check if the area belongs to a city or directly to the county
        ...
        county_city_url = get_and_modify_county_coordinates_url(county, (x, y))
