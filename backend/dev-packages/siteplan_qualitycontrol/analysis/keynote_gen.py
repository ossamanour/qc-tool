"""
Perform keynote generation module.
"""
import os
...
def keynote_generate(
    ...
    """
    Detect keynote list from siteplan image.
    ...
    # read in the config file and get the keynote template name
    with open(os.path.join(Path(orig_img_path).parents[1], "config.json"), "r") as fp:
        config = json.load(fp)
        keynote_template_name = config["keynote_template"]
...
def keynote_generate_post(
        ...
        index: int = None):
    """
    Return keynote detections results on siteplan, for all or one.

    @param: orig_img_path: string of path the originla PNG image for the siteplan.
    @param: index: if None, return information for all detected keynotes. if not, return keynote [index].
    """
    # save folder path
    data_save_path = os.path.join(Path(orig_img_path).parent, "data")
    img_save_path = os.path.join(Path(orig_img_path).parent, "image")
