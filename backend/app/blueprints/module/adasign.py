...
@module_bp.route("/module/ada-sign", methods=["POST"])
def ada_sign():
    module_name = "ada-sign"
    print(module_name)
    # perform task
    ...
    if return_response["status"] == "succeed":
        message.append(f"Detected ADA sign(s): {return_response["total_number"]}")
    
        current_page = Page.query.filter_by(session_id=session["currentSession"]["id"], page_number=session["currentPage"]).first()
        session["currentDisplay"] = os.path.join(current_page.page_folder, "image", "ada_sign.png")
    else:
        message.append("No ADA sign detected.")
